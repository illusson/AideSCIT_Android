...
class UpdateModule {
    fun getUpdate(callback: Callback) {
        val call: Call = BaseAPI().getUpdateRequest("release")
        call.enqueue(object : okhttp3.Callback {
            override fun onFailure(call: Call, e: IOException) {
                if (e is UnknownHostException) {
                    callback.onUpdateFailure(-701, Application.getString(R.string.error_network), e)
                } else {
    ...
    fun handleDownload(dlUrl: String) {
        ...
        val dir = Application.APPLICATION_CONTEXT.getExternalFilesDir("update")?.path
        val downloadManager = Application.APPLICATION_CONTEXT
            .getSystemService(Context.DOWNLOAD_SERVICE) as DownloadManager
        val req = DownloadManager.Request(url)
        req.setNotificationVisibility(DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED)
        val apkName: String = Application.APPLICATION_CONTEXT
            .getString(R.string.app_name) + "_" + BaseAPI.TS + ".apk"
