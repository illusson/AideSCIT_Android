...
class UpdateHelper(val context: Context) {
    ...
    fun getUpdate(type: Int, callback: Callback) {
        val version: String = if (type == 1) {
            "debug"
        } else {
            "release"
        }
        val call: Call = APIHelper().getUpdateRequest(version)
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                try {
                    ...
                    if (verCode > verCodeNow) {
                        val urlDl = "https://tool.eclass.sgpublic.xyz/update/apk/app-$version.apk"
                        var isForce = 0
                        if (version == "release") {
                            isForce = if (updateTable.getLong("force") > verCodeNow) 1 else 0
                        }
                        val sizeString: String = getSizeString(urlDl)
                        callback.onUpdate(
                            isForce,
                            updateTable.getString("ver_name"),
                            sizeString,
                            updateTable.getString("changelog"),
                            urlDl
                        )
                    } else {
                        callback.onUpToDate()
